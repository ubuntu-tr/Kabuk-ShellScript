...
then
    ...
    if echo "$nmcli" | grep -q "wifi";
    then
	echo $(nmcli radio wifi on)
	echo wifi is up and runnning
    elif echo "$nmcli" | grep -q "wwan"
    then
	 echo $(nmcli radio wwan on)
	 echo wwan is up and running
    else
	echo $(nmcli radio all on)
	echo all network is up and runnning
    fi
